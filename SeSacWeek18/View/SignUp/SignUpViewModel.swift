...
extension SignUpViewModel: CommonViewModel {
    ...
    func transfrom(input: Input) -> Output {
        let userNameValidation = input.userNameText.map { $0.count >= 2 }
        let emailValidation = input.emailText.withUnretained(self)
            .map { (vc, value) in
                vc.isValidEmail(value: value)
            }
        let passwordValidation = input.userNameText.map { $0.count >= 8 }
        
        return Output(userNameTextField: input.userNameTextField.orEmpty, emailTextField: input.emailTextField.orEmpty, passwordTextField: input.passwordTextField.orEmpty, signUpTap: input.signUpTap, userNameValidation: userNameValidation, emailValidation: emailValidation, passwordValidation: passwordValidation)
    }
}
